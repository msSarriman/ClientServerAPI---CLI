...
//************************** Listen & Accept process code START ************************ //
    ...
    *tableRow=0;
    printf("Talbe row is ------------> <%d>\n", *tableRow);
    while(1)
    {   
        //printf("My port is    <%d>.\n", ntohs(serverAddr.sin_port));
        #ifdef _debug
            printf("MAIN: I am now waiting:\n");        
            clock_t tac = clock();
            printf("MAIN Elapsed: %f seconds\n", (double)(tac - tic) / CLOCKS_PER_SEC);
        #endif
        clientSocket    = accept(mySocket, (struct sockaddr *) &cli_addr, &addrlen);        // Create a dedicated socket for the incoming connection. The new socket is used to communicate with the client.
        pid_t pid;  // creation of a child process
        pid = fork();
        if(pid==0)
        {
            printf("Talbe row is -----KID----> <%d>\n", *tableRow);

            close(listenVar);
            initFunc();
            shmdt((void*)kvptr);
            shmdt((void*)tableRow);
            printf("GOODBYE CRUEL WORLD pid <%d>\n", getpid());
            printf("GOODBYE CRUEL WORLD pid <%d>\n", getpid());
            printf("GOODBYE CRUEL WORLD pid <%d>\n", getpid());
            printf("GOODBYE CRUEL WORLD pid <%d>\n", getpid());
            exit(1);
